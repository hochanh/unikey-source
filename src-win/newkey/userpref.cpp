...
int CUserPref::getValue()
{
	...
	if (m_switchKey >= 2 || m_switchKey < 0)
		m_switchKey = 0;
	if (m_otherCharset < 3 || m_otherCharset >= TOTAL_CHARSET)
		m_otherCharset = 3;
	if (m_codeTable > 2 && m_codeTable != m_otherCharset)
		m_codeTable = m_otherCharset;
	if (m_inMethod > VNI_INPUT)
		m_inMethod = TELEX_INPUT;
...
int enableAutoStart(int enabled)
{
	...
	if (enabled) {
		...
		res = RegSetValueEx(key, STARTUP_REG_VALUENAME, 0, REG_SZ, (LPBYTE)path, _tcsclen(path));
